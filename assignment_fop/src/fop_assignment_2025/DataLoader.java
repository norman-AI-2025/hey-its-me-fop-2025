            ...
            while ((line = br.readLine()) != null) {
                ...
                if (data.length >= 4) { 
                    Employee emp = (data.length >= 5)
                        ...
                        : new Employee(data[0].trim(), data[1].trim(), data[2].trim(), data[3].trim());
                    employees.add(emp);
                }
            }
...
// --- E. Load Sales ---
    private void loadSales() {
        try (BufferedReader br = new BufferedReader(new FileReader(FilePathHelper.resolveReadPath(SALES_FILE_PATH)))) {
            String header = br.readLine(); // header (may vary by dataset)
            String line;
            ...
            while ((line = br.readLine()) != null) {
                ...
                // A) Our system format: SalesID,Date,CustomerName,ModelName,Quantity,TotalPrice
                // B) Provided CSV format: Date,CustomerName,Model,TotalAmount,PaymentMethod,EmployeeID
                try {
                    if (data.length >= 6 && data[0].trim().matches("S\\d+")) {
                        // Format A
