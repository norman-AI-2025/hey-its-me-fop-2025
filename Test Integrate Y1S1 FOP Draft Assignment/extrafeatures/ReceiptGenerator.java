...
public class ReceiptGenerator {
    ...
    public void generateStockReceipt(String type, String fromOutlet, String toOutlet, String employeeName, List<String> models, List<Integer> quantities) {
        ...
        // Example: "receipts/receipts_2025-11-29.txt"
        LocalDate today = ZonedDateTime.now(ZoneId.of("Asia/Kuala_Lumpur")).toLocalDate();
        String filename = "receipts/receipts_" + today.toString() + ".txt";
    ...
    public void generateSalesReceipt(String customerName, String modelName, int quantity, double totalMetrics, String paymentMethod, String employeeName) {
        ...
        LocalDate today = ZonedDateTime.now(ZoneId.of("Asia/Kuala_Lumpur")).toLocalDate();
        String filename = "receipts/sales_" + today.toString() + ".txt";
        ...
        try (PrintWriter pw = new PrintWriter(new FileWriter(filename, true))) {
            ...
            System.out.println(" [Receipt] Sales receipt generated: " + filename);
        } catch (IOException e) {
            System.err.println(" ! Error generating sales receipt: " + e.getMessage());
        }
    }

    //Generate Stock Transfer Receipt (array overload for GUI)
    public void generateStockReceipt(String type, String fromOutlet, String toOutlet, String employeeName,
                                     String[] models, int[] quantities, int size) {
        LocalDate today = ZonedDateTime.now(ZoneId.of("Asia/Kuala_Lumpur")).toLocalDate();
        String filename = "receipts/receipts_" + today.toString() + ".txt";
        ...
        int totalQuantity = 0;
        for (int i = 0; i < size; i++) totalQuantity += quantities[i];
        ...
        try (PrintWriter pw = new PrintWriter(new FileWriter(filename, true))) {
            ...
            for (int i = 0; i < size; i++) {
                pw.println("- " + models[i] + " (Qty: " + quantities[i] + ")");
            }
            pw.println("--------------------------------");
            pw.println("Total Quantity: " + totalQuantity);
            pw.println("--------------------------------\n");
    ...
    //Generate Sales Receipt (array overload for GUI)
    public void generateSalesReceipt(String customerName, String[] modelNames, int[] quantities, double[] unitPrices,
                                     int size, String paymentMethod, String employeeName) {
        LocalDate today = ZonedDateTime.now(ZoneId.of("Asia/Kuala_Lumpur")).toLocalDate();
        String filename = "receipts/sales_" + today.toString() + ".txt";
        ...
        double total = 0.0;
        for (int i = 0; i < size; i++) total += unitPrices[i] * quantities[i];
        ...
        try (PrintWriter pw = new PrintWriter(new FileWriter(filename, true))) {
            ...
            pw.println("Items:");
            for (int i = 0; i < size; i++) {
                pw.println("- " + modelNames[i] + " x" + quantities[i] + " @ RM" + unitPrices[i]);
            }
